...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(swPerson => {
    let $copy = $template.clone();
    $copy.find('h2').text(swPerson.name);
    $copy.find('h3').text(swPerson.height);
    $copy.find('p').text(swPerson.eye_color);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ing, index) => {
    let tempArray = [];
    tempArray = recipe.ingredients.slice(index,index+1);
    let tempStr = tempArray[0];
