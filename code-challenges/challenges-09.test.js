...
function getCurrentEvents(request, response){
  response.status(200).send(mapCurrentEvents(currentEvents));
}

const mapCurrentEvents = () => {
  let returnArray = [];
  returnArray = currentEvents.news.map((val) => new Event(val.author, val.category, val.published, val.image, val.date, val.title)
  );

  return returnArray;
};

function Event(author, categories, summary, img_url, date, title){
  this.author = author;
...
const countNumberOfElements = (arr) => {
  let returnCount = arr.reduce((counter,value,indx) => {
    counter++;
    return counter;
  }, 0);
  return returnCount;
...
const returnNames = (arr) => {
  let returnArray = arr.reduce(function(accumulator, elem){
    accumulator.push(elem.name);
    return accumulator;
  }, []);
...
const reversedString = (str) => {
  let forwStr = str.split('');
  let revStr = forwStr.reduce((accStr, letter) => {
